	...
	public List<Pair<Integer, Point>> getMoves(Integer num_moves, HashMap<Integer, Point> player_pieces, HashMap<Integer, Point> opponent_pieces, boolean isplayer1)
	{
		...
		if(n <= 20) {
			 for(int index = n-1; index >= 0 && moves.size()!=num_moves; index--) {
				 ...
				 if (index!=0){index--;}
				 int piece_id2 = pointsByX.get(index).getKey();
				 ...
				 Pair<Integer, Point> move2 = new Pair<Integer, Point>(piece_id2, new_position);
				 ...
				 if(check_validity(move2, player_pieces, opponent_pieces)) {
					...
					double x = isplayer1 ? new_position.x - diameter_piece : new_position.x + diameter_piece;
					move2 = new Pair<Integer, Point>(piece_id2, new Point(x, new_position.y));
					if(moves.size()!=num_moves && check_validity_(move2, player_pieces, opponent_pieces)) {
						moves.add(move2);
			 ...
			 for(int piece_id = 0; piece_id < n && moves.size()!=num_moves; piece_id++) {
				 ...
				 Point curr_position = player_pieces.get(piece_id);
				 if (piece_id<n){piece_id++;}
				 Point curr_position2 = player_pieces.get(piece_id);
				 Point new_position = new Point(curr_position);
				 Point new_position2 = new Point(curr_position2);
				 new_position.x = isplayer1 ? new_position.x - diameter_piece : new_position.x + diameter_piece;
				 new_position2.x = isplayer1 ? new_position2.x - diameter_piece : new_position2.x + diameter_piece;
				 
				 Pair<Integer, Point> move = new Pair<Integer, Point>(piece_id, new_position);
				 Pair<Integer, Point> move2 = new Pair<Integer, Point>(piece_id, new_position2);
				 if(check_validity(move, player_pieces, opponent_pieces))
					 moves.add(move);
				 if(check_validity(move2, player_pieces, opponent_pieces))
				  	moves.add(move2);
		...
		// if n >= 15,  use wall
			...
			if(attack_points.size() != 0 ) {
				int frunner= attack_points.keySet().stream().findFirst().get(); //keySet().stream().findFirst().get();
					int wall_index = attack_points.get(frunner);
					...
					if(check_validity(move, player_pieces, opponent_pieces)) {
						...
						move = new Pair<Integer, Point>(frunner, temp);
						moves.add(move);
						wall_point.put(frunner, temp);
						...
						attack_points.remove(frunner);
						ignored_piece.add(frunner);
						back_wall_point.put(frunner, temp);
						return moves;
					}
			}
